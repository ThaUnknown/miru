...
async function getRSSEntries ({ media, episode, mode, ignoreQuality }) {
  ...
  const url = new URL(
    `https://nyaa.si/?page=rss&c=1_2&f=0&s=seeders&o=desc&q=(${titles})${ep}${quality}-(${excl})`
  )
  ...
  if (absolute) {
    ...
    const url = new URL(
      `https://nyaa.si/?page=rss&c=1_2&f=0&s=seeders&o=desc&q=(${titles})${epstring(absoluteep)}${quality}-(${excl})`
    )
